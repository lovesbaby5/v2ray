...
update_v2ray() {
	...
	echo
	echo -e " 无法执行更新, 固定 V2Ray 版本为: v4.45.2"
	echo
}
update_v2ray.sh() {
	if [[ $_test ]]; then
		...
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/233boy/v2ray/old/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
